...
class TorncityservicesClubApplicationTests {
	...
	static void tearDownTests() {
		MongoTemplate mongoTemplate = StaticContextAccessor
	...
	public void testTornPlayerDataFetch() {
		...
		log.info(
				"testTornPlayerDataFetch() -> player data fetched from torn {}",
				player);
	}
	...
	public void testTornPlayerEventsFetch() {
		...
		log.info("testTornPlayerEventsFetch() --> total events fetched {}",
				eventsDTO.getEvents().size());
	...
	public void testRegisterPlayer() {
		...
		Player fetchedPlayer = playerService
				.getPlayerByPlayerTornId(tornUserId);
		Assert.notNull(fetchedPlayer, "failed to fetch torn player from db!");
		player = fetchedPlayer;
		log.info("testRegisterPlayer()--> registered player fetched! {}",
				fetchedPlayer);
	}
	...
	public void testFetchingJobs() {
		...
		List<JobResponseDTO> foundJobs = jobService.getJobsByFilter(filterReq)
				.getJobs();
	...
	public void testExactingMoneyFromEventMessage() {
		...
		Assert.isTrue(amountExtracted.equals(550_000L),
				"unable to extract correct amount from message!");
	}
