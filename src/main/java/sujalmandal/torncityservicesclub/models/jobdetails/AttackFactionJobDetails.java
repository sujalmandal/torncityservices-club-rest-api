...
public class AttackFactionJobDetails implements JobDetails {
    ...
    @FormField(label = "Total attacks on faction members", formatter = FieldFormatterValue.NUMBER, type = FormFieldTypeValue.NUMBER)
    ...
    @FormField(label = "Total duration in days over which the attack has to take place", formatter = FieldFormatterValue.NUMBER, type = FormFieldTypeValue.NUMBER)
    @FilterableField(label = "total duration in days", maxFieldLabel = "maximum no. of days", minFieldLabel = "minimum no. of days")
    private String duration;

    @HighlightWhen
    @FormField(label = "Total pay for this job", type = FormFieldTypeValue.NUMBER, formatter = FieldFormatterValue.CURRENCY)
