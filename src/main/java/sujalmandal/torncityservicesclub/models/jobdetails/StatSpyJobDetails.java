...
public class StatSpyJobDetails implements JobDetails {
    ...
    @HighlightWhen
    @FormField(label = "Total pay", formatter = FieldFormatterValue.CURRENCY, type = FormFieldTypeValue.NUMBER, maxValue = "5_000_000")
    @FilterableField(label = "total pay per spy", maxFieldLabel = "maximum pay per spy", minFieldLabel = "minimum pay per spy")
    private String pay;

    @FormField(label = "Total spies to sell", type = FormFieldTypeValue.NUMBER, serviceType = ServiceTypeValue.OFFER, minValue = "1", maxValue = "25")
