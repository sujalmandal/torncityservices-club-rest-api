...
public class JobController implements AuthenticatedController {
    ...
    public ResponseEntity<?> postJob(@RequestBody
    ...
    public ResponseEntity<?> acceptJob(@RequestBody
    JobUpateRequestDTO request) {
	this.injectPlayerInRequest(request);
    ...
    public ResponseEntity<?> finishJob(@RequestBody
    JobUpateRequestDTO request) {
	this.injectPlayerInRequest(request);
    ...
    public ResponseEntity<?> cancelJob(@RequestBody
    JobUpateRequestDTO request) {
	this.injectPlayerInRequest(request);
    ...
    JobFilterRequestDTO request) {
	this.injectPlayerInRequest(request);
	request.setFilterFields(null);
	request.setFetchDetails(Boolean.FALSE);
    ...
    public ResponseEntity<?> advancedSearch(@RequestBody
    JobFilterRequestDTO request) {
	this.injectPlayerInRequest(request);
	request.setFetchDetails(Boolean.FALSE);
	return ResponseEntity.ok().body(jobService.getJobsByFilter(request));
    ...
    public ResponseEntity<?> getJobDetails(@PathVariable(
	"id"
    )
    Long id) {
	JobFilterRequestDTO request = new JobFilterRequestDTO();
	request.setJobIds(Collections.singletonList(id));
	request.setFetchDetails(Boolean.TRUE);
	this.injectPlayerInRequest(request);
	return ResponseEntity.ok().body(jobService.getJobsByFilter(request));
    }
