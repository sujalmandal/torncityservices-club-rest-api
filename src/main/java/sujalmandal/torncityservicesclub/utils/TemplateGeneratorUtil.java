...
public class TemplateGeneratorUtil {
	...
	private static final Reflections reflections = new Reflections(
			JOB_DETAIL_IMPL_SEARCH_PACKAGE.toString());
	
	private static Set<Class<?>> jobDetailImplClasses;
	
	public static Set<JobDetailFormTemplate> generateJobDetailFormTemplates()
		...
		for (Class<?> clazz : jobDetailImplClasses) {
			...
			String formTemplateName = clazz
					.getAnnotation(GenerateTemplate.class).value()
					.getFormTemplateName();
			String formTemplateLabel = clazz
					.getAnnotation(GenerateTemplate.class).value()
					.getFormTemplateLabel();
			String filterTemplateName = clazz
					.getAnnotation(GenerateTemplate.class).value()
					.getFilterTemplateName();
			String filterTemplateLabel = clazz
					.getAnnotation(GenerateTemplate.class).value()
			...
			for (Field field : clazz.getDeclaredFields()) {
				...
				if (field.isAnnotationPresent(FormField.class)) {
					...
					throw new ServiceException(String.format(
							"{%s} of {%s} missing a mandatory annotation!",
	...
	private static void extractFormFieldDescriptor(Field field,
	...
	public static Set<JobDetailFilterTemplate> generateJobDetailFilterTemplates()
			throws JsonProcessingException {
		TemplateGeneratorUtil.loadJobDetailImplClasses();
		...
		for (Class<?> clazz : jobDetailImplClasses) {
			...
			for (Field field : clazz.getDeclaredFields()) {
				...
				boolean isFilterable = field
						.isAnnotationPresent(FilterableField.class);
				if (isFilterable) {
					if (field.isAnnotationPresent(FormField.class)) {
						FormField formField = field
								.getAnnotation(FormField.class);
						FilterableField filterableField = field
								.getAnnotation(FilterableField.class);
						...
						switch (fieldType) {
							case NUMBER :
								long maxValue = getNumber(formField.maxValue());
								long minValue = getNumber(formField.minValue());
								extractNumberFilterFields(filterTemplate,
										fieldType, filterableField, fieldName,
										serviceType, format, defaultValue,
										maxValue, minValue);
								break;
							case CHECKBOX :
								extractCheckboxFilterField(filterTemplate,
										fieldType, fieldName, serviceType,
										fieldLabel, defaultValue);
								break;
							case SELECT :
								String[] options = formField.options();
								extractSelectFilterField(filterTemplate,
										fieldType, fieldName, serviceType,
										fieldLabel, defaultValue, options);
								break;
							default :
								extractTextFilterField(filterTemplate,
										fieldType, fieldName, serviceType,
										fieldLabel, defaultValue);
						}
	...
	private static void extractSelectFilterField(
			JobDetailFilterTemplate filterTemplate, FormFieldTypeValue type,
			String fieldName, ServiceTypeValue serviceType, String fieldLabel,
	...
	private static void extractCheckboxFilterField(
			JobDetailFilterTemplate filterTemplate, FormFieldTypeValue type,
			String fieldName, ServiceTypeValue serviceType, String fieldLabel,
			String defaultValue) {
		FilterFieldDescriptor fieldDescriptor = new FilterFieldDescriptor();
	...
	private static void extractTextFilterField(
			JobDetailFilterTemplate filterTemplate, FormFieldTypeValue type,
			String fieldName, ServiceTypeValue serviceType, String fieldLabel,
			String defaultValue) {
		FilterFieldDescriptor fieldDescriptor = new FilterFieldDescriptor();
	...
	private static void extractNumberFilterFields(
			JobDetailFilterTemplate filterTemplate, FormFieldTypeValue type,
			FilterableField filterableField, String fieldName,
			ServiceTypeValue serviceType, FieldFormatterValue format,
	...
	private static long getNumber(String decoratedNumberAsString) {
		return Long.parseLong(decoratedNumberAsString.replaceAll("_", ""));
	}
	...
	public static <V, K> void loadJobDetailImplClasses() {
		if (jobDetailImplClasses == null) {
			...
			TemplateGeneratorUtil.jobDetailImplClasses = Collections
					.unmodifiableSet(reflections
							.getTypesAnnotatedWith(GenerateTemplate.class));
		}
	}
